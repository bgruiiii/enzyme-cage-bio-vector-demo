...
def compute_ec3_grouped_metrics(query_emb, corpus_emb, ec3_labels_query,
    ...
    Returns:
        dict with keys: n_queries, MRR, top-1, top-5, top-10
    """
    ...
    for ci, start in enumerate(range(0, n_query, chunk_size)):
        ...
        for local_i in range(end - start):
            ...
            gset = set(gi.tolist())
            accum["top1"]  += int(bool(set(topk_idx[:1].tolist()) & gset))
            accum["top5"]  += int(bool(set(topk_idx[:5].tolist()) & gset))
            accum["top10"] += int(bool(set(topk_idx[:10].tolist()) & gset))
            accum["n"] += 1
...
def main():
    ...
    if all_pass:
        ...
        below_reference = [
            k for k, v in threshold_ratings.items() if v["rating"] == "FAIL"
        ]
        conclusion = (
            f"{n_pass}/{n_total} thresholds passed. "
            f"Metrics below reference: {', '.join(below_reference)}. "
